...
/**
 * The original Reports body — every analytics card plus the filter bar. Lives
 * in its own component so the Reports page can route between this and the
 * Monthly Performance tab via `?tab=`. Analytics is the default (no `?tab=`),
 * so its filter/pagination links keep working unchanged — there is no `tab`
 * param to thread through them.
 */
export async function AnalyticsTab({
  ...
  return (
    <>
      ...
      <Card
        title="Time in stage"
        description="Median + p90 days submissions sit in each pipeline stage before moving. Terminal states (Rejected / On Hold / Joined / Backed Out) excluded. Ongoing counts how many samples are still sitting in that stage right now."
      >
