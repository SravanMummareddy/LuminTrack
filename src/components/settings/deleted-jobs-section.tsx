...
/**
 * Admin "recycle bin" for permanently-erased jobs — the backup zip written just
 * before each job was erased. Download to inspect, or remove for good. Mirrors
 * DeletedCandidatesSection.
 */
export function DeletedJobsSection({
  archives,
}: {
  archives: JobArchiveEntry[];
}) {
  return (
    <section className="space-y-4">
      <div>
        ...
        <p className="mt-0.5 text-xs text-slate-500">
          Backups saved when a job was permanently erased. Download one to inspect
          it, or remove it for good to make it unrecoverable.
        </p>
      </div>

      {archives.length === 0 ? (
        <div className="rounded-lg border border-dashed border-slate-300 bg-white p-8 text-center text-sm text-slate-500">
          No erased-job backups.
        </div>
      ) : (
                ...
                <tr key={a.pathname} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-2.5">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <ConfirmSubmit
                        action={removeJobArchive}
                        fields={{ pathname: a.pathname, url: a.url }}
                        title={`Remove backup ${a.displayId} for good?`}
                        description="This permanently removes the backup zip. After this, the job's data is unrecoverable."
